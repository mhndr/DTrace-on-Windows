...
void dtrace_debug_output() {
}

__inline pid_t dtrace_pidof(proc_t* p) {
    return (pid_t)(ULONG_PTR)PsGetProcessId((PEPROCESS)p);
}

__inline proc_t* ttoproc(kthread_t* t) {
    NT_ASSERT(t == curthread);
    return (proc_t*)PsGetCurrentProcess();
}
...
uint32_t       	dtrace_strsize_default = 256;
...
#define	DTRACE_TLS_THRKEY(where) { \
	uint_t intr = mstate->dtms_ipl; \
	ASSERT(intr < 16); \
	(where) = (((uint64_t)KeGetCurrentThread() << 4) + intr); \
}
	...
	for (i = 0; i < dtrace_toxranges; i++) {			\
		...
		*flags |= CPU_DTRACE_BADADDR;				\
		...
		return (0);						\
									...
									\
	uint##bits##_t rval;						\
	if (dtrace_safememcpy(&rval, addr, size, size, TRUE)) {		\
		return rval;						\
	}       							\
									\
...
static dtrace_helpers_t *dtrace_helpers_get(proc_t *);
...
void dtrace_setpredcache(intptr_t value)
{
#ifdef _WIN32
    PULONG_PTR p = dtrace_threadprivate(0);
    if (NULL != p) {
        *p = (ULONG_PTR)value;
    }
#else
    curthread->t_predcache = value;
#endif
}

int dtrace_matchpredcache(intptr_t value)
{
#ifdef _WIN32
    PULONG_PTR p = dtrace_threadprivate(0);
    if (NULL == p) {
        ...
        return (intptr_t)*p == value;
    }
#else
    return curthread->t_predcache == value;
#endif
}
...
#else
		return ((uint64_t)dtrace_pidof(curproc));
#endif
...
#if defined(_WIN32)
		return (uint64_t)PsGetProcessInheritedFromUniqueProcessId(PsGetCurrentProcess());
#elif defined(illumos)
...
#ifdef _WIN32
		return (uint64_t)PsGetCurrentThreadId();
#else
...
#if defined(_WIN32)
		return (dtrace_dif_varstr(
		    (uintptr_t) PsGetProcessImageFileName(PsGetCurrentProcess()), state, mstate));
#elif defined(illumos)
		if (!dtrace_priv_proc(state))
			return (0);
    ...
    dtrace_mstate_t *mstate, dtrace_state_t *state)
{
	...
	case DIF_SUBR_PROGENYOF: {
		...
		DTRACE_CPUFLAG_SET(CPU_DTRACE_NOFAULT);
#ifdef _WIN32
		if (pid == (pid_t)dtrace_pidof(curproc)) {
			rval = 1;
		}
#else
	...
	case DIF_SUBR_BASENAME: {
		...
		uint64_t size = state->dts_options[DTRACEOPT_STRSIZE];
		uintptr_t src = tupregs[0].dttk_value;
		int i, j, len = (int)dtrace_strlen((char *)src, size);
...
dtrace_probe(dtrace_id_t id, uintptr_t arg0, uintptr_t arg1,
    uintptr_t arg2, uintptr_t arg3, void* ctx)
{
	processorid_t cpuid;
	dtrace_icookie_t cookie;
	...
	if (!onintr && probe->dtpr_predcache != DTRACE_CACHEIDNONE &&
	    dtrace_matchpredcache(probe->dtpr_predcache)) {
		/*
...
#ifdef _WIN32
	/*
	 * The 'cookie' returned by the interrupt_disable() on NTOS
	 * is the original IRQL.
	 */
	mstate.dtms_ipl = cookie;
	mstate.dtms_present = DTRACE_MSTATE_IPL;
#endif
	...
	for (ecb = probe->dtpr_ecb; ecb != NULL; ecb = ecb->dte_next) {
		...
		*flags &= ~CPU_DTRACE_ERROR;
		...
		if (ecb->dte_cond) {
			...
			if ((ecb->dte_cond & DTRACE_COND_USERMODE) &&
#ifdef _WIN32
			    prov->dtpv_pops.dtps_getframe(prov->dtpv_arg,
			            probe->dtpr_id, probe->dtpr_arg, ctx,
				    NULL, NULL) != UserMode)
#else
			    prov->dtpv_pops.dtps_usermode(prov->dtpv_arg,
			            probe->dtpr_id, probe->dtpr_arg, ctx) == 0)
#endif
				continue;
		...
		for (act = ecb->dte_action; !(*flags & CPU_DTRACE_ERROR) &&
			...
			case DTRACEACT_STACK:
				...
				dtrace_getpcstack((pc_t *)(tomax + valoffs),
				    (int)(size / sizeof (pc_t)), probe->dtpr_aframes,
			...
			case DTRACEACT_USTACK:
				...
				if (DTRACE_USTACK_STRSIZE(rec->dtrd_arg) != 0 &&
				    dtrace_helpers_get(curproc) != NULL) {
					/*
...
#ifdef illumos
				pid = curthread->t_procp->p_pidp->pid_id;
#else
				pid = dtrace_pidof(curproc);
#endif
...
dtrace_errdebug(const char *str)
{
	...
	dtrace_panic("dtrace: undersized error hash");
out:
...
#ifdef _WIN32
		    (void (*)(void *, const char*, const char*, const char*, const char*))dtrace_nullop;
#else
...
dtrace_unregister(dtrace_provider_id_t id)
{
	...
	if ((prev = dtrace_provider) == old) {
		...
		if (prev == NULL) {
			dtrace_panic("attempt to unregister non-existent "
			    "dtrace provider %p\n", (void *)id);
		}
    ...
    const char *func, const char *name)
{
	...
	pkey.dtpk_mod = (char*)mod;
	pkey.dtpk_mmatch = mod ? &dtrace_match_string : &dtrace_match_nul;
	pkey.dtpk_func = (char*)func;
	pkey.dtpk_fmatch = func ? &dtrace_match_string : &dtrace_match_nul;
	pkey.dtpk_name = (char*)name;
...
dtrace_probe_provide(dtrace_probedesc_t *desc, dtrace_provider_t *prv)
{
	...
	do {
		 ...
		 */
#ifdef _WIN32
		prv->dtpv_pops.dtps_provide(prv->dtpv_arg,
					    (desc ? desc->dtpd_provider : NULL),
					    (desc ? desc->dtpd_mod : NULL),
					    (desc ? desc->dtpd_func : NULL),
					    (desc ? desc->dtpd_name : NULL));
#else
...
{
	char *fmt, **new;
	uint16_t ndx, len = (uint16_t)strlen(str) + 1;
	...
	for (act = ecb->dte_action; act != NULL; act = act->dta_next) {
		...
		if (DTRACEACT_ISAGG(act->dta_kind)) {
			...
			ecb->dte_size = P2ROUNDUP(ecb->dte_size,
			    rec->dtrd_alignment);
			rec->dtrd_offset = (uint32_t)ecb->dte_size;
...
dtrace_ecb_aggregation_create(dtrace_ecb_t *ecb, dtrace_actdesc_t *desc)
{
	...
	agg->dtag_action.dta_rec.dtrd_size = (uint32_t)size;
...
dtrace_ecb_action_add(dtrace_ecb_t *ecb, dtrace_actdesc_t *desc)
{
	...
	if (DTRACEACT_ISAGG(desc->dtad_kind)) {
		...
		case DTRACEACT_STACK:
			...
			size = (uint32_t)(nframes * sizeof (pc_t));
			break;
		...
		case DTRACEACT_USTACK:
			 ...
			 */
			size = (uint32_t)((nframes + 1) * sizeof (uint64_t));
    ...
    dtrace_enabling_t *enab)
{
	...
	if (dtrace_ecb_create_cache != NULL) {
		...
		dtrace_action_t *action = cached->dte_action;

		if (action != NULL) {
			ASSERT(action->dta_refcnt > 0);
			action->dta_refcnt++;
			ecb->dte_action = action;
...
retry:
		...
		for (enab = dtrace_retained; enab != NULL;
			...
			for (i = 0; i < enab->dten_ndesc; i++) {
				...
				mutex_exit(&dtrace_lock);
#ifdef _WIN32
				prv->dtpv_pops.dtps_provide(parg,
							    desc.dtpd_provider,
							    desc.dtpd_mod,
							    desc.dtpd_func,
							    desc.dtpd_name);
...
#else
dtrace_state_create(struct cdev *dev, void *cred __unused)
#endif
{
#ifdef illumos
	minor_t minor;
	...
	{
		BOOLEAN KernelAccess = TRUE;
		BOOLEAN UserAccess = TRUE;
		dtrace_priv_filter((ARGUMENT_PRESENT(dev)
				    ? ExGetPreviousMode()
				    : KernelMode),
				   &KernelAccess, &UserAccess);
...
dtrace_state_prereserve(dtrace_state_t *state)
{
	...
	for (ecb = probe->dtpr_ecb; ecb != NULL; ecb = ecb->dte_next) {
		...
		state->dts_reserve += (uint_t)(ecb->dte_needed + ecb->dte_alignment);
	}
}
...
#ifdef _WIN32
	callout_cleanup(&state->dts_cleaner);
	callout_cleanup(&state->dts_deadman);
#endif
#endif
    ...
    dtrace_state_t *state, uint64_t arg0, uint64_t arg1)
{
	...
	uint64_t rval = 0;
	dtrace_helpers_t *helpers = dtrace_helpers_get(curproc);
    ...
    dof_helper_t *dofhp)
{
	...
	if (!dtrace_attached() || dtrace_meta_pid == NULL) {
		...
		dtrace_helper_provide(dofhp, dtrace_pidof(p));
		...
		if (dtrace_meta_pid != NULL) {
			...
			for (i = 0; i < help->dthps_nprovs; i++) {
				dtrace_helper_provider_remove(
				    &help->dthps_provs[i]->dthp_prov, dtrace_pidof(p));
			}
		} else {
			mutex_enter(&dtrace_lock);
...
#elif defined(_WIN32)
dtrace_module_unloaded(const char* modname)
#else
...
#elif defined(_WIN32)
	template.dtpr_mod = (char*)modname;
#endif
...
#elif defined(_WIN32)
int
dtrace_open(PDEVICE_OBJECT dev, dtrace_state_t ** data)
#endif
{
	dtrace_state_t *state;
	uint32_t priv;
...
#else
	state = dtrace_state_create((struct cdev *)dev, NULL);
#if defined(__FreeBSD__)
...
#elif defined(_WIN32)
void
dtrace_close(dtrace_state_t *data)
#endif
{
#ifdef illumos
...
dtrace_load(void)
{
	...
	dtrace_taskq = taskq_create("dtrace_taskq", 1, -1, 0, 0, 0);
...
{
	struct {int fltoffs; uintptr_t illval;} ctx;
...
dtrace_ioctl(struct dtrace_state *state, unsigned long cmd, void* arg)
{
	...
	case DTRACEIOC_CONF: {
		...
		rval = copyout(&conf, arg, sizeof(conf));
		return (rval == 0 ? 0 : EFAULT);
	}
	case DTRACEIOC_DOFGET: {
		dof_hdr_t hdr, *dof;
	...
	case DTRACEIOC_ENABLE: {
		...
		if (copyin(arg, &enable, sizeof (enable)) != 0)
			return (EFAULT);
		...
		if ((dof = dtrace_dof_copyin((uintptr_t) enable.dof, &rval)) == NULL)
			return (EINVAL);
		...
		if ((err = dtrace_enabling_match(enab, &enable.n_matched)) == 0) {
			err = dtrace_enabling_retain(enab);
		} else {
			dtrace_enabling_destroy(enab);
		}
		...
		if (copyout(&enable, arg, sizeof (enable)) != 0)
			return (EFAULT);
	...
	case DTRACEIOC_EPROBE: {
		...
		epdesc.dtepd_probeid = ecb->dte_probe->dtpr_id;
		epdesc.dtepd_uarg = ecb->dte_uarg;
		epdesc.dtepd_size = (uint32_t)ecb->dte_size;
...
#ifdef _WIN32
			desc.dtargd_mapping = prov->dtpv_pops.dtps_getargdesc(
				prov->dtpv_arg, probe->dtpr_id, probe->dtpr_arg,
				desc.dtargd_ndx, desc.dtargd_native,
				sizeof(desc.dtargd_native));
			if (DTRACE_ARGNONE == desc.dtargd_mapping) {
				desc.dtargd_ndx = DTRACE_ARGNONE;
			}
#else
			prov->dtpv_pops.dtps_getargdesc(prov->dtpv_arg,
