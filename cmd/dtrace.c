...
bootdof_add(void)
{
	// TODO: boot time tracing for NTOS is not currently implemented.
	return;
}
...
{
#ifdef _WIN32
	HANDLE h = proc_gethandle(P);
	DWORD pid = proc_getpid(P);
	DWORD status;
...
static LPTOP_LEVEL_EXCEPTION_FILTER unhandledexceptionfilter_org;
static LONG WINAPI
unhandledexceptionfilter(struct _EXCEPTION_POINTERS *ExceptionInfo)
{
	/* breakin when debugger is attached */
	__try{__debugbreak();}__except(EXCEPTION_EXECUTE_HANDLER){}
	if (unhandledexceptionfilter_org)
		return unhandledexceptionfilter_org(ExceptionInfo);
...
static void
enable_privileges(void)
{
	HANDLE Token;
	ULONG i;

	/*
	 * LookupPrivilegeValue is not available on OneCore
	 * use LUID values instead. Values are documented as part of the WDK.
	 */
	...
	*/
	...
	struct {
	    ...
	    LUID_AND_ATTRIBUTES Array[RTL_NUMBER_OF(Privileges) -
				      RTL_NUMBER_OF_FIELD(TOKEN_PRIVILEGES,
							  Privileges)];
	} s = {0};

	for (i = 0; i < RTL_NUMBER_OF(Privileges); i += 1) {
		PLUID_AND_ATTRIBUTES Entry =
		    &s.Header.Privileges[s.Header.PrivilegeCount];

		/*
		if (!LookupPrivilegeValue(NULL, Privileges[i], &Entry->Luid)) {
			continue;
		}
		*/

		Entry->Luid.LowPart = Privileges[i];
		Entry->Attributes = SE_PRIVILEGE_ENABLED;
		s.Header.PrivilegeCount += 1;
	}
	...
	if (OpenThreadToken(GetCurrentThread(), TOKEN_ADJUST_PRIVILEGES, TRUE, &Token) ||
		...
		AdjustTokenPrivileges(Token, FALSE, &s.Header, 0, NULL, NULL);
		CloseHandle(Token);
	}
}
...
#ifdef _WIN32
	/*
	 * Access checks in dtrace driver are based on the state of
	 * TCB, debug, and loaddriver privileges.
	 * this code will try to enable them it they are in the current
	 * effective token.
	 */
	enable_privileges();
#endif
